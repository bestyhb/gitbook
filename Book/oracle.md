...
## Cursor (游标)
### Cursor解释
Cursor本质上就是一块内存区域，由系统或用户以变量的形式定义，可以提高数据处理的效率。
Cursor将查询出的结果集（多条记录）统一存放至内存中，由游标从内存中获取数据。
Cursor能够解决一次性查询多条记录的问题，所以处理多行数据的事务经常使用游标实现。
### Cursor生命周期
创建。定义游标，分配内存区域（内存中没有任何数据）；
开启。存入游标，将结果集存入内存；
提取。
关闭。关闭游标，释放内存资源。
### Cursor使用
